...
├── simulator/                    # 테스트 도구 (저울 흉내, 배포 X)
│   ├── simulator.py              #   가상 저울 (칭량 시나리오 + 제조사 선택)
│   └── scale_formats.py          #   제조사별 송신 형식 (CAS/AND)
├── arduino_scale_simulator/
│   └── arduino_scale_simulator.ino   # 아두이노 스케치 (실물 신호 검증)
├── logs/                         # 로그 출력 폴더 (.log 는 git 제외)
├── config.json                   # [1일차] 무게표시기 설정
...
> 진입점(`pop_main`, `main`)에 src 를 경로에 추가하는 부트스트랩이 있어 **파일 직접 실행**이 된다.
> 패키지 안에서는 `from common.scale_parser import ...` 처럼 `common.`/`pop.`/`display.` 로 import 한다.
...
python simulator/simulator.py COM5

# 터미널 B — 내 프로그램(반대쪽 COM6 수신)
python src/display/main.py
```
...
python -m PyInstaller --paths src --onefile --windowed --name 저울데이터수집프로그램 src/display/main.py
```

- `--paths src` : 패키지(common/display/pop) 를 찾도록 src 를 검색 경로에 추가
- `--onefile` : 파일 하나(.exe)로 묶기
- `--windowed` : 콘솔창 없이 GUI 만 실행
- `--name` : 결과 exe 이름
- `python -m PyInstaller` : `pyinstaller` 명령이 PATH 에 없을 때도 동작

> **POP만 배포**하려면 진입점을 `src/pop/pop_main.py` 로 빌드한다(아래 "확장" 참고).
> import 를 따라가므로 무게표시기·시뮬레이터는 자동으로 빠진다.
...
(코드가 frozen 상태에서는 exe 위치 기준으로 config·logs 를 찾는다 — `src/common/paths.py`)
...
> 기본 모드(`python src/display/main.py`)는 그대로 두고, **`python src/pop/pop_main.py`** 라는
...
| 파일 | 역할 |
|------|------|
| `simulator/scale_formats.py` | 제조사별 송신 형식 (CAS / A&D) — 시뮬레이터가 사용 |
| `src/pop/judge.py` | 목표±허용오차 판정 (부족/양호/초과, 차이·오차%) — 순수 함수 |
| `src/pop/recipe.py` + `recipe.json` | 배합비 (원료·목표·허용·저울포트). DB 교체 시 `load_recipe()` 만 수정 |
| `src/pop/pop_gui.py` / `src/pop/pop_main.py` | 배합 칭량 POP 화면 / 진입점 |
...
```
python -m PyInstaller --paths src --onefile --windowed --name 배합칭량 src/pop/pop_main.py
```

→ `dist/배합칭량.exe` 옆에 **`recipe.json`** 을 두면 배포 끝 (config.json 대신 recipe.json).
